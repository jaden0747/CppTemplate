...
inline bool drawCombo(const std::string& key, nlohmann::json& val,
...
inline bool drawResetButton(const nlohmann::json* defVal, nlohmann::json& val)
{
...
inline bool drawField(const std::string& key, nlohmann::json& val,
...
inline void draw(const char* title, bool* pOpen = nullptr)
{
    ...
    {
        if (searchBuf[0] != '\0' && !detail::icontains(itemName, searchBuf))
            continue;
    ...
    if (it != items.end())
    {
        ...
        for (auto& [key, val] : j.items())
        {
            ...
            if (detail::drawField(key, val, def, opts)) changed = true;
        }
