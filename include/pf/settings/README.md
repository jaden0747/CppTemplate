...
- **`pf::SettingsRegistry`** — singleton that maps string keys to typed settings objects
- **`pf::SettingsItem<T>`** — RAII wrapper that auto-registers a settings struct at static-init time
- **`pf::DirtyTracker`** — mixin base for change-count bookkeeping and side-effect callbacks
- **`pf::buildRootMenu()`** — builds a `daniele77/cli` menu exposing the full registry over a local terminal or Telnet
...
```cpp
// examples/include/demo/app_config.hpp
...
Inherit `pf::DirtyTracker` to get a `modifiedCount` that increments on every load/set, and override `onChanged()` for custom side effects:

```cpp
// examples/include/demo/render_settings.hpp
...
Declare an `inline pf::SettingsItem<T>` next to the struct in its header. Registration happens at static-init time — before `main()` runs — and `inline` keeps it to one shared definition across every binary that includes the header:

```cpp
// examples/include/demo/app_config.hpp
inline pf::SettingsItem<AppConfig>      g_app{"AppConfig"};       // key = "AppConfig"
...
If the item has `pf::DirtyTracker`, `dirty()` is called.
...
change callbacks and `pf::DirtyTracker` fire. In the ImGui editor this is surfaced as
...
`pf::buildRootMenu()` (in `command_registry.hpp`) creates a `cli::Menu` with a `coding` submenu that exposes the full registry over a local terminal and optionally over Telnet.
...
```
include/pf/settings/          — shipped as part of the pf package
    dirty_tracker.hpp         — pf::DirtyTracker mixin base
    settings_registry.hpp     — pf::SettingsRegistry singleton
    settings_item.hpp         — pf::SettingsItem<T> RAII wrapper
include/pf/cli/
    command_registry.hpp      — pf::buildRootMenu() for CLI server
include/pf/ui/
    settings_editor.hpp       — pf::SettingsEditor::draw() ImGui panel

src/pf/settings/              — pf::settings implementation
src/pf/cli/                   — pf::cli implementation

examples/include/demo/        — NOT shipped; your project writes its own
    app_config.hpp            — flat scalars, inherits pf::DirtyTracker
    render_settings.hpp       — array + string enum, inherits pf::DirtyTracker
    network_config.hpp        — nested sub-object (EndpointInfo)

app/
    main.cpp                  — ImGui app
    cli_server.cpp            — Telnet CLI server

resources/
    settings.xml              — default overrides (copied to build output)
```

> The snippets above are written unqualified, as you would write them in your
> own project. In *this* repo the example structs live in `namespace demo`
> (`demo::AppConfig`, `demo::g_app`) to keep the app/platform boundary visible —
> the registry key strings (`"AppConfig"`, and the matching XML element names)
> are unaffected by that namespace.
